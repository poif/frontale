...
unsigned long calculate_SHA256 ( char* input )
{
    ...
    {
        fprintf ( stderr, "Erreur_calculate_SHA256 : chaine nulle " ) ;
        return ERRNO ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_calculate_SHA256 : SHA256_Init " ) ;
        return ERRNO ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_calculate_SHA256 : SHA256_Update " ) ;
        return ERRNO ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_calculate_SHA256 : SHA256_Final " ) ;
        return ERRNO ;
    }
...
int generate_RSA_keys ()
{
    ...
    {
        fprintf ( stderr, "Erreur_generate_RSA_keys : RSA_New() " ) ;
    ...
    if ( ( bne = BN_new() ) == NULL )
    {
        fprintf ( stderr, "Erreur_generate_RSA_keys : BN_new() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        return ERRNO ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_generate_RSA_keys : BN_set_word() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        return ERRNO ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_generate_RSA_keys : RSA_generate_key_ex() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        return ERRNO ;
    }
    ...
    if ( bdd_key == NULL )
    {
        fprintf ( stderr, "Erreur_generate_RSA_keys : impossible de generer les cles " ) ;
        return ERRNO ;
    }
    else
        return TRUE ;
...
int RSA_chiffrement ( unsigned char* input, unsigned char* output, int mode )
{
    ...
    {
        fprintf ( stderr, "Erreur_RSA_chiffrement : invalid RSA keys " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        return ERRNO ;
    }
    ...
    if ( mode == CHIFFREMENT )
    {
        ...
        {
            fprintf ( stderr, "Erreur_RSA_chiffrement : error chiffrement " ) ;
            fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
            return ERRNO ;
        }
    }
    else if ( mode == DECHIFFREMENT )
    {
        ...
        {
            fprintf ( stderr, "Erreur_RSA_dechiffrement : error déchiffrement " ) ;
            fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
            return ERRNO ;
        }
    }
    ...
    {
        fprintf ( stderr, "Erreur_RSA_chiffrement : bad mode " ) ;
        return ERRNO ;
    }
...
int RSA_recup_key_pub ( unsigned char* key )
{
    ...
    {
        fprintf ( stderr, "Erreur_RSA_recup_key_pub : BIO_new_mem_buf() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        return ERRNO ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_RSA_recup_key_pub : PEM_read_bio_RSA_PUBKEY() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        return ERRNO ;
    }
...
int AES_chiffrement ( char* input, char* output, int len, int* ilen, int mode )
{
    ...
    {
        free ( out ) ;
        fprintf ( stderr, "Erreur_AES_chiffrement : EVP_CipherInit_ex() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
    ...
    {
        free ( out ) ;
        fprintf ( stderr, "Erreur_AES_chiffrement : EVP_CipherUpdate() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        EVP_CIPHER_CTX_cleanup ( &ctx ) ;
        return ERRNO ;
    }
    ...
    {
        free ( out ) ;
        fprintf ( stderr, "Erreur_AES_chiffrement : EVP_CipherFinal_ex() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        EVP_CIPHER_CTX_cleanup ( &ctx ) ;
        return ERRNO ;
    }
    ...
    *ilen = olen1 + olen2 ;
    ...
    {
        fprintf ( stderr, "Erreur_AES_chiffrement : EVP_CIPHER_CTX_cleanup() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        return ERRNO ;
    }
...
int AES_envoi_key_aes ( int id )
{
    ...
    while ( good != TRUE )
    {
        ...
        {
            fprintf ( stderr, "Erreur_AES_envoi_key_aes : RSA_chiffrement " ) ;
            free ( chiffre ) ;
            free ( cle_aes ) ;
            return ERRNO ;
        }
...
/*int RSA_envoi_key_pub ()
{
    ...
    {
        fprintf ( stderr, "Erreur_RSA_envoi_key_pub : BIO_new() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        free ( key ) ;
        return ERRNO ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_RSA_envoi_key_pub : PEM_write_bio_RSAPublicKey() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        free ( key ) ;
        BIO_free ( keybio ) ;
        return ERRNO ;
    ...
    {
        fprintf ( stderr, "Erreur_RSA_envoi_key_pub : BIO_read() " ) ;
        fprintf ( stderr, "Code erreur OpenSSL : %lu \n ", ERR_get_error () ) ;
        free ( key ) ;
        BIO_free ( keybio ) ;
        return ERRNO ;
