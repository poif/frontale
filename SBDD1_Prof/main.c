    ...
    if ( mysql_bdd != NULL )
    {
        ...
        if ( AES_generate_key() != ERRNO )
        {
            ...
            if ( generate_RSA_keys() != ERRNO )
            {
                    ...
                    if ( ( return_value = res_activation() ) != ERRNO )
                    {
                        ...
                        if ( ( return_value = pthread_create ( &thread_receive, NULL, fct_thread_res_receive, NULL ) ) == 0 )
                        {
                            ...
                            if ( ( pthread_cancel ( thread_receive ) ) != 0 )
                                fprintf ( stderr, "Erreur THREAD : fermeture du thread ! " ) ;
                        }
                        else
                            fprintf ( stderr, "Erreur THREAD : impossible lancer le thread " ) ;
                    }
                ...
                {
                    EVP_cleanup () ;    // Finalisation OpenSSL
                    fprintf ( stderr, "Erreur BDD : impossible de vider la BDD => Pensez à exécuter bdd.sql " ) ;
                    return EXIT_FAILURE ;
                }
            }
            else
            {
                EVP_cleanup () ;    // Finalisation OpenSSL
                fprintf ( stderr, "Erreur BDD : impossible de générer clé RSA " ) ;
                return EXIT_FAILURE ;
            }
        }
        else
        {
            EVP_cleanup () ;    // Finalisation OpenSSL
            fprintf ( stderr, "Erreur BDD : impossible de générer clé AES " ) ;
            return EXIT_FAILURE ;
        }
    }
    else
