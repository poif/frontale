...
MYSQL* bdd_start_connection ()
{
    ...
    {
        fprintf ( stderr, "Erreur_bdd_start_connection : mysql_init false " ) ;
        return NULL ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_bdd_start_connection : mysql_options " ) ;
        return NULL ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_bdd_start_connection : mysql_real_connect => Avez-vous exécuter bdd.sql ? " ) ;
        return NULL ;
    }
...
int ref_concatenation ( Ref* reference, char* reference_finale )
{
    ...
    for ( i = 0; i < 4; i++ )
    {
        ...
        switch ( i )
        {
            ...
            default :
                fprintf ( stderr, "Erreur_ref_concatenation : overflow compteur i " ) ;
                free ( tmp ) ;
                return ERRNO ;
                break ;
        }
        ...
        {
            case ZERO :
                fprintf ( stderr, "Erreur_ref_concatenation : tmp taille nulle " ) ;
                free ( tmp ) ;
                return ERRNO ;
                break ;
            ...
            default :
                fprintf ( stderr, "Erreur_ref_concatenation : overflow compteur taille " ) ;
                free ( tmp ) ;
                return ERRNO ;
                break ;
        }
...
int ref_split ( char* reference_input, Ref* reference )
{
    ...
    for ( i = 0; i < 4; i++ )
    {
        ...
        switch ( i )
        {
            ...
            default :
                fprintf ( stderr, "Erreur_ref_split : overflow compteur i " ) ;
                free ( tmp ) ;
                return ERRNO ;
                break ;
        }
        ...
        if ( errno != 0 )
        {
            fprintf ( stderr, "Erreur_ref_split : strtol " ) ;
            fprintf ( stderr, "%s \n", strerror ( errno ) ) ;
            free ( tmp ) ;
            return ERRNO ;
        }
...
int ref_cmp ( Ref* reference, Ref* modele )
{
    ...
    for ( i = 0; i < 4; i++ )
    {
        ...
        switch ( i )
        {
            ...
            default :
                fprintf ( stderr, "Erreur_ref_cmp : overflow compteur i " ) ;
                return ERRNO ;
                break ;
        }
    }
...
int bdd_verification_partage ( char* politiques, char* atester )
{
    ...
    {
        fprintf ( stderr, "Erreur_bdd_verification_partage : strtok_r I " ) ;
    ...
    while ( morceau != NULL )
    {
        ...
        {
            fprintf ( stderr, "Erreur_bdd_verification_partage : strtok_r II " ) ;
            free ( duplicata_politiques ) ;
            return ERRNO ;
        }
...
void bdd_send_msg ( int id, int flag, char* texte, int chiffre )
{
    ...
    if ( chiffre == TRUE )
    {
        ...
        {
            fprintf ( stderr, "Erreur_bdd_send_msg : AES_chiffrement " ) ;
            free ( out ) ;
            free ( trame ) ;
        }
        else
...
int bdd_send_reponse ( char *texte, int id, int code )
{
    ...
    {
        fprintf ( stderr, "Erreur_bdd_send_reponse : AES_chiffrement " ) ;
        free ( reponse ) ;
        free ( out ) ;
        return ERRNO ;
    }
