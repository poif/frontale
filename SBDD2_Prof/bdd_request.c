...
int bdd_parser ( char *trame )
{
    ...
    for ( i = 0; i < 2; i++ )
    {
        ...
        switch ( i )
        {
            ...
            default :
                fprintf ( stderr, "Erreur_bdd_parser : Overflow i " ) ;
                if ( do_free == TRUE )
                    free ( clair ) ;
    ...
    switch ( flag )
    {
        case INSERT :
            if ( traitement == TRUE )
            {
                ...
                {
                    fprintf ( stderr, "Erreur_bdd_parser : bdd_do_insert() " ) ;
                    if ( do_free == TRUE )
                        free ( clair ) ;
                    return ERRNO ;
                }
            ...
            {
                fprintf ( stderr, "Erreur_bdd_parser : bad ini " ) ;
                if ( do_free == TRUE )
                    free ( clair ) ;
        ...
        case DELETE :
            if ( traitement == TRUE )
            {
                ...
                {
                    fprintf ( stderr, "Erreur_bdd_parser : bdd_do_delete() " ) ;
                    if ( do_free == TRUE )
                        free ( clair ) ;
                    return ERRNO ;
                }
        ...
        case SEEK :
            if ( traitement == TRUE )
            {
                ...
                {
                    fprintf ( stderr, "Erreur_bdd_parser : bdd_do_seek() " ) ;
                    if ( do_free == TRUE )
                        free ( clair ) ;
                    return ERRNO ;
                }
        ...
        case SELECT :
            if ( traitement == TRUE )
            {
                ...
                {
                    fprintf ( stderr, "Erreur_bdd_parser : bdd_do_select() " ) ;
                    if ( do_free == TRUE )
                        free ( clair ) ;
                    return ERRNO ;
                }
        ...
        case INIT :
            ...
            {
                fprintf ( stderr, "Erreur_bdd_parser : bad ini 2 " ) ;
                if ( do_free == TRUE )
                    free ( clair ) ;
        ...
        default :
            fprintf ( stderr, "Erreur_bdd_parser : flag incorrect " ) ;
            if ( do_free == TRUE )
                free ( clair ) ;
            bdd_send_msg ( id, flag, "ERROR", TRUE ) ;
            return ERRNO ;
...
int bdd_do_init ( int id, char *safe_trame )
{
    ...
    {
        fprintf ( stderr, "Erreur_bdd_do_init : information manquante " ) ;
    ...
    {
        fprintf ( stderr, "Erreur_bdd_do_init : information manquante II " ) ;
        bdd_send_msg ( id, INIT, "ERROR", FALSE ) ;
        return ERRNO ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_bdd_do_init : RSA_recup_key_pub() " ) ;
        bdd_send_msg ( id, INIT, "ERROR", FALSE ) ;
        return ERRNO ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_bdd_do_init : AES_envoi_key_aes() " ) ;
        bdd_send_msg ( id, INIT, "ERROR", FALSE ) ;
        return ERRNO ;
    }
...
int bdd_do_insert ( int id, char *safe_trame )
{
    ...
    for ( cpt = 0; cpt < 7; cpt++ )
    {
        ...
        if ( ( morceau = strtok_r ( NULL, "*", &safe_trame ) ) == NULL )
        {
            fprintf ( stderr, "Erreur_bdd_do_insert : information manquante " ) ;
        ...
        {
            fprintf ( stderr, "Erreur_bdd_do_insert : information manquante II " ) ;
            if ( politiques_free == TRUE )
                free ( politiques ) ;
            if ( groupes_free == TRUE )
                free ( groupes ) ;
        ...
        else
        {
            ...
            switch ( cpt )
            {
                ...
                default :
                    fprintf ( stderr, "Erreur_do_insert : overflow compteur cpt " ) ;
                    if ( politiques_free == TRUE )
                        free ( politiques ) ;
                    if ( groupes_free == TRUE )
                        free ( groupes ) ;
    ...
    if ( cpt != 4 )
    {
        fprintf ( stderr, "Erreur_bdd_do_insert : politiques() " ) ;
        free ( politiques ) ;
        free ( groupes ) ;
        bdd_send_msg ( id, INSERT, "ERROR", TRUE ) ;
        return ERRNO ;
    ...
    do
    {
        ...
        if ( morceau != NULL )
        {
            ...
            {
                fprintf ( stderr, "Erreur_bdd_do_insert : ref_concatenation() " ) ;
                free ( politiques ) ;
                free ( reference_str ) ;
                free ( valeur_finale ) ;
                free ( groupes ) ;
            ...
            {
                fprintf ( stderr, "Erreur_bdd_do_insert : bdd_do_request() " ) ;
                free ( politiques ) ;
                free ( reference_str ) ;
                free ( valeur_finale ) ;
                free ( groupes ) ;
    ...
    {
        fprintf ( stderr, "Erreur_do_insert : bdd_send_reponse() " ) ;
        free ( politiques ) ;
        free ( reference_str ) ;
        free ( valeur_finale ) ;
        free ( groupes ) ;
...
int bdd_do_delete ( int id, char *safe_trame )
{
    ...
    for ( cpt = 0; cpt < 2; cpt++ )
    {
        ...
        if ( ( morceau = strtok_r ( NULL, "*", &safe_trame ) ) == NULL )
        {
            fprintf ( stderr, "Erreur_bdd_do_delete : information manquante " ) ;
        ...
        if ( strcmp ( morceau, "EOF" ) == 0 )
        {
            fprintf ( stderr, "Erreur_bdd_do_delete : information manquante II" ) ;
            bdd_send_msg ( id, DELETE, "ERROR", TRUE ) ;
            return ERRNO ;
        }
        else
        {
            ...
            switch ( cpt )
            {
                ...
                default :
                    fprintf ( stderr, "Erreur_do_delete : overflow compteur cpt " ) ;
                    bdd_send_msg ( id, DELETE, "ERROR", TRUE ) ;
                    return ERRNO ;
                    break ;
            }
    ...
    {
        fprintf ( stderr, "Erreur_bdd_do_delete : bdd_search_nom() " ) ;
        free ( user_ref ) ;
        bdd_send_msg ( id, DELETE, "ERROR", TRUE ) ;
        return ERRNO ;
    }
    ...
    if ( strcmp ( user, user_ref ) == 0 )
    {
        ...
        {
            fprintf ( stderr, "Erreur_bdd_do_delete : bdd_do_request() " ) ;
            free ( user_ref ) ;
            bdd_send_msg ( id, DELETE, "ERROR", TRUE ) ;
            return ERRNO ;
        }
...
int bdd_do_seek ( int id, char *safe_trame )
{
    ...
    for ( cpt = 0; cpt < 4; cpt++ )
    {
        ...
        if ( ( morceau = strtok_r ( NULL, "*", &safe_trame ) ) == NULL )
        {
            fprintf ( stderr, "Erreur_bdd_do_seek : information manquante " ) ;
        ...
        if ( strcmp ( morceau, "EOF" ) == 0 )
        {
            fprintf ( stderr, "Erreur_bdd_do_seek : information manquante II" ) ;
            bdd_send_msg ( id, SEEK, "ERROR", TRUE ) ;
            return ERRNO ;
        }
        else
        {
            ...
            switch ( cpt )
            {
                ...
                default :
                    fprintf ( stderr, "Erreur_do_seek : overflow compteur cpt " ) ;
                    bdd_send_msg ( id, SEEK, "ERROR", TRUE ) ;
                    return ERRNO ;
                    break ;
            }
    ...
    {
        fprintf ( stderr, "Erreur_bdd_do_seek : bdd_search_ref() " ) ;
        return ERRNO ;
    }
...
int bdd_do_select ( int id, char *safe_trame )
{
    ...
    for ( cpt = 0; cpt < 4; cpt++ )
    {
        ...
        if ( ( morceau = strtok_r ( NULL, "*", &safe_trame ) ) == NULL )
        {
            fprintf ( stderr, "Erreur_bdd_do_select : information manquante " ) ;
        ...
        if ( strcmp ( morceau, "EOF" ) == 0 )
        {
            fprintf ( stderr, "Erreur_bdd_do_select : information manquante II" ) ;
            bdd_send_msg ( id, SELECT, "ERROR", TRUE ) ;
            return ERRNO ;
        }
        else
        {
            ...
            switch ( cpt )
            {
                ...
                default :
                    fprintf ( stderr, "Erreur_do_select : overflow compteur cpt " ) ;
                    bdd_send_msg ( id, SELECT, "ERROR", TRUE ) ;
                    return ERRNO ;
                    break ;
            }
    ...
    {
        fprintf ( stderr, "Erreur_bdd_do_select : bad requete " ) ;
    ...
    {
        fprintf ( stderr, "Erreur_bdd_do_select : bad tuple " ) ;
        mysql_free_result ( resultat_req ) ;
        bdd_send_msg ( id, SELECT, "ERROR", TRUE ) ;
        return ERRNO ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_bdd_do_select : RSA_dechiffrement " ) ;
        free ( output ) ;
        mysql_free_result ( resultat_req ) ;
        bdd_send_msg ( id, SELECT, "ERROR", TRUE ) ;
        return ERRNO ;
    ...
    for ( cpt = 0; cpt < 3; cpt++ )
    {
        ...
        if ( morceau == NULL )
        {
            fprintf ( stderr, "Erreur_bdd_do_select : information manquante III " ) ;
            free ( output ) ;
            mysql_free_result ( resultat_req ) ;
            bdd_send_msg ( id, SELECT, "ERROR", TRUE ) ;
            return ERRNO ;
    ...
    for ( cpt = 0; cpt < 3; cpt++ )
    {
        ...
        if ( morceau == NULL )
        {
            fprintf ( stderr, "Erreur_bdd_do_select : information manquante IV " ) ;
            free ( output ) ;
            mysql_free_result ( resultat_req ) ;
            bdd_send_msg ( id, SELECT, "ERROR", TRUE ) ;
            return ERRNO ;
    ...
    {
        fprintf ( stderr, "Erreur_do_select : bdd_send_reponse() " ) ;
        free ( output ) ;
        mysql_free_result ( resultat_req ) ;
        bdd_send_msg ( id, SELECT, "ERROR", TRUE ) ;
        return ERRNO ;
...
int bdd_search_nom ( char* reference, char* nom )
{
    ...
    if ( bdd_do_request ( mysql_bdd, SELECT, reference, NULL, &resultat_req ) == ERRNO )
    {
        fprintf ( stderr, "Erreur_bdd_search_nom : bad requete " ) ;
        mysql_free_result ( resultat_req ) ;
        return ERRNO ;
    }
    ...
    if ( ( tuple = mysql_fetch_row ( resultat_req ) ) ==  NULL )
    {
        fprintf ( stderr, "Erreur_bdd_search_nom : bad tuple " ) ;
        mysql_free_result ( resultat_req ) ;
        return ERRNO ;
    }
    ...
    if ( RSA_chiffrement ( (unsigned char*) tuple[0], (unsigned char*) output, DECHIFFREMENT ) != TRUE )
    {
        fprintf ( stderr, "Erreur_bdd_search_nom : RSA_dechiffrement " ) ;
        free ( output ) ;
        mysql_free_result ( resultat_req ) ;
        return ERRNO ;
    }
...
int bdd_search_ref ( int id, Ref* modele )
{
    ...
    {
        fprintf ( stderr, "Erreur_bdd_search_ref : bad requete " ) ;
        mysql_free_result ( resultat_req ) ;
        bdd_send_msg ( id, SEEK, "ERROR", TRUE ) ;
        return ERRNO ;
    }
    ...
    {
        fprintf ( stderr, "Erreur_bdd_search_ref : bad search " ) ;
        mysql_free_result ( resultat_req ) ;
        return ERRNO ;
    }
...
int bdd_search_modele ( int id, MYSQL_RES* resultat_req, Ref* modele )
{
    ...
    while ( suite == TRUE )
    {
        ...
        if ( suite == TRUE )
        {
            ...
            {
                fprintf ( stderr, "Erreur_bdd_search_modele : ref_split " ) ;
                free ( user ) ;
                free ( temporaire ) ;
                free ( output ) ;
                bdd_send_msg ( id, SEEK, "ERROR", TRUE ) ;
            ...
            {
                fprintf ( stderr, "Erreur_bdd_traitement_request : ref_cmp " ) ;
                free ( user ) ;
                free ( temporaire ) ;
                free ( output ) ;
                bdd_send_msg ( id, SEEK, "ERROR", TRUE ) ;
            ...
            else if ( return_value == TRUE )    // La référence nous intéresse
            {
                ...
                {
                    fprintf ( stderr, "Erreur_bdd_serach_modele : RSA_dechiffrement " ) ;
                    free ( output ) ;
                    free ( user ) ;
                    free ( temporaire ) ;
                    bdd_send_msg ( id, SEEK, "ERROR", TRUE ) ;
    ...
    if ( suite != ERRNO )
    {
        ...
        {
            fprintf ( stderr, "Erreur_do_seek : bdd_send_reponse() " ) ;
            free ( user ) ;
            free ( output ) ;
            free ( temporaire ) ;
            bdd_send_msg ( id, SEEK, "ERROR", TRUE ) ;
    ...
    {
        fprintf ( stderr, "Erreur_bdd_serach_modele : aucun tuple " ) ;
        free ( user ) ;
        free ( output ) ;
        free ( temporaire ) ;
        bdd_send_msg ( id, SEEK, "ERROR", TRUE ) ;
...
int bdd_do_request ( MYSQL* mysql_bdd, int action, char* reference, char* valeur, MYSQL_RES** resultat_req )
{
    ...
    if ( action == INSERT )
    {
        ...
        if ( strlen ( valeur ) <= ( TAILLE_MAX_UTILE - 1 ) )
        {
            ...
            while ( good != TRUE )
            {
                ...
                {
                    fprintf ( stderr, "Erreur_bdd_do_request : RSA_chiffrement " ) ;
                    free ( insertion ) ;
                    free ( requete ) ;
                    return ERRNO ;
                }
    ...
    switch ( action )
    {
        ...
        default :
            fprintf ( stderr, "Erreur_bdd_do_request : bad action " ) ;
            free ( requete ) ;
            if ( insertion != NULL )
                free ( insertion ) ;
            return ERRNO ;
    ...
    if ( ( action == SELECT_ALL ) || ( action == SELECT ) )
    {
        ...
        {
            fprintf ( stderr, "Erreur_bdd_do_request : mysql_use_result " ) ;
            free ( requete ) ;
            if ( insertion != NULL )
                free ( insertion ) ;
            return ERRNO ;
